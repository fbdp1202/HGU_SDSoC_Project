...
#else
//        net->workspace = calloc(1, workspace_size);
        net->workspace = (float *)sds_alloc(workspace_size);
#endif
    }
    return net;
}
...
float* load_input(network *net){
    fprintf(stderr, "load output to layer.inputs\n");
    FILE *fp = fopen("layer.inputs", "rb");
    if(!fp) file_error("layer.inputs");
    layer l = net->layers[0];
    float *result = (float*)malloc(sizeof(float)*l.inputs);
    fread(result, sizeof(float), l.inputs, fp);
    fclose(fp);
    return result;
}

void compare_input(network *net){
    float *result = load_input(net);
    int i, cnt, max_loop;
    float *input = net->input;
    layer l = net->layers[0];
    max_loop = 1000;
    cnt = 0;
    for(i=0; i < l.inputs && i < max_loop; i++){
//        if((result[i] - input[i]) > 0.00001f || (result[i] - input[i]) < -0.00001f){
        if(result[i] != input[i]){
            printf("result[%d]=%.38f, input[%d]=%.38f\n",i, result[i], i, input[i]);
            cnt++;
            if(cnt > 1000)
                break;
        }
...
float* load_output(network *net, int index){
    char filename[40] = {0};
    sprintf(filename, "layer_%d.outputs", index);
    fprintf(stderr, "load output to %s\n",filename);
    FILE *fp = fopen(filename, "rb");
    if(!fp) file_error(filename);

    layer l = net->layers[index];
    float *result = (float*)malloc(sizeof(float)*l.outputs);
    fread(result, sizeof(float), l.outputs, fp);
    fclose(fp);
    return result;
}

void compare_result(network *net, int index){
    float *result = load_output(net, index);
    int i,cnt, max_loop;
    layer l = net->layers[index];
    max_loop = 1000;
    cnt = 0;
    for(i = 0; i < l.outputs && i < max_loop; i++){
        printf("result[%d]=%.38f, l.output[%d]=%.38f\n", i, result[i], i, l.output[i]);
        if((result[i] -l.output[i]) > 0.00001f || (result[i] -l.output[i]) < -0.00001f){
            cnt++;
            if(cnt > 1000){
                break;
            }
...
void load_weights_upto(network *net, char *filename, int start, int cutoff)
{
    ...
    if ((major*10 + minor) >= 2 && major < 1000 && minor < 1000){
        printf("cond1, sizeof(size_t) = %d, ", sizeof(size_t));
    ...
    fprintf(stderr, "Done!\n");
    fclose(fp);
    //load_variance_weights(net, start, cutoff);
}
