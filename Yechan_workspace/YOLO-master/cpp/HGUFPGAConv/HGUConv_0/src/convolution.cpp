...
void sw_convolutional_layer(layer *pl, float *inputs, int num_repeat) {
	...
	double im2col_cpu_time = what_time_is_it_now();
	printf("CPU im2col Time: %f seconds\n", im2col_cpu_time-start_cpu_time);

	for (i=0; i<num_repeat; i++) {
		gemm(m,n,k,a,k,b,n,c,n);
	}
	printf("CPU gemm convolution Time: %f seconds\n", (what_time_is_it_now()-im2col_cpu_time)/num_repeat);
	free(b);
}
...
void test_convolution() {
	...
	unsigned char *inputs = (unsigned char *)calloc(input_size,sizeof(unsigned char));
	// unsigned char *inputs;
	// inputs = (unsigned char *)calloc(input_size, sizeof(unsigned char));
	set_random_input(inputs, input_size);

	float *X = (float *)calloc(input_size, sizeof(float));
...
void hw_convolutional_layer(layer *pl, float *inputs, int num_repeat) {
	...
	double im2col_fpga_time = what_time_is_it_now();
	printf("FPGA im2col Time: %f seconds\n", im2col_fpga_time-start_fpga_time);


	for (i=0; i<num_repeat; i++) {
		gemm_fpga(m,n,k,a,k,b,n,c,n);
	}
	printf("FPGA imge convolution Time: %f seconds\n", (what_time_is_it_now()-im2col_fpga_time)/num_repeat);
}
#endif // SDSOC