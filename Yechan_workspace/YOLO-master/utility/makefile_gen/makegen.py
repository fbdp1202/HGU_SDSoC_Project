...
def create_params(target):
    ...
    target.write("# FPGA Board Platform (Default ~ zed)\n")
...
def add_flags(target, data):
    ...
    target.write("EXECUTABLE := darknet.elf ")
    target.write("\n")
...
def add_objs(target, extension, flag):
    count = 0
    ...
    for result in glob.glob(extension):
        temp -= 1
        target.write("$(pwd)/$(BUILD_DIR)/src/")
        if extension == "*.cpp":
...
def add_srcs(target, data):
    ...
    add_objs(target, extension_cpp, 0)
    add_objs(target, extension_c, 1)
...
def add_kernel_flags(target, data):
    ...
    target.write("SCC := sdscc $(SDSFLAGS)\n")
    target.write("\n")
    return

def mk_build_all(target):
    ...
    target.write("$(pwd)/$(BUILD_DIR)/src/%.o: $(pwd)/$(SRC_DIR)/%.c\n")
    target.write("\t@echo 'Building file: $<'\n")
    target.write("\t@echo 'Invoking: SDSCC Compiler'\n")
    target.write("\tmkdir -p $(BUILD_DIR)\n")
    target.write("\tcd $(BUILD_DIR) ; $(SCC) $(CFLAGS) -o $(LFLAGS) $(HW_FLAGS)\n")
    ...
    target.write("$(pwd)/$(BUILD_DIR)/src/%.o: $(pwd)/$(SRC_DIR)/%.cpp\n")
    target.write("\t@echo 'Building file: $<'\n")
